...
> **Final-freeze addendum:** EXP-010 is now the primary scientific result; the safe headline is **45/50 paired wins** from `joint_sweep.json`. The baseline values below are local reference-protocol results, not exact reproduction of organiser-saved RF outputs. Existing evidence does not identify the discrepancy cause because package/input provenance is incomplete. No independent replication of EXP-010 is claimed. Gate4: E1 FALSIFIED, E2 INCONCLUSIVE (+0.004953 < +0.0100), E3 FALSIFIED; E3's full independent validator stopped at `Original tree dirty` before scientific reconstruction.
...
- **CORRECTION (appended, not rewritten) — the blocker above was MISDIAGNOSED and is RESOLVED. No action is needed from Jaswin.**
  - **What was actually wrong:** I have two GitHub accounts. Git was presenting the cached credential for `BuanyomiAmetor`, while the account with access to the team repo is **`AmetorB`**. GitHub returned 403 (authenticated but unauthorised) because it was refusing the *wrong account*, not refusing me. Repository permissions were never missing.
  - **Why I got it wrong:** I inspected Windows Credential Manager, found a single stored GitHub credential for `BuanyomiAmetor`, and concluded the account must be correct. A single stored credential does not mean a single account exists — that inference was unsound, and I stated the wrong-account explanation was ruled out when it had not been.
  - **Fix applied:** set the remote to `https://AmetorB@github.com/...` so Credential Manager resolves the correct account. Push then succeeded immediately with no prompt and no permission change.
  - **Do NOT ask Jaswin for collaborator access on my behalf, and do not assume Members 3 and 4 are blocked** — that recommendation rested on the misdiagnosis. If they do hit a 403, the first thing to check is which GitHub account their credential manager is presenting.

- **Commit authorship corrected before publishing.** Both commits were originally authored `Ametor Buanyomi <167139278+BuanyomiAmetor@users.noreply.github.com>` — the wrong account, because `git config user.email` pointed at the other profile. Push authentication and commit authorship are independent, so this would have shown my baseline work under an account not on the team roster, which matters under the 30-point Presentation & Team Collaboration criterion. Both commits were re-authored to `a.h.buanyomi@gmail.com` (AmetorB) **before** the first push, so no shared history was rewritten. File content verified byte-identical across the rewrite (empty diff against the pre-rewrite tip). Identity was set **repo-local**, not global, to avoid disturbing the other account's repositories.

- **`core.longpaths` enabled (repo-local).** The first re-author attempt failed with `Filename too long`: the repository sits 187 characters deep inside nested OneDrive folders, and git's rebase temp files exceeded the Windows 260-character `MAX_PATH` limit. Enabling long-path support fixed it. Expect this to recur for any member running git operations from this OneDrive location — `git config core.longpaths true` is the fix.
