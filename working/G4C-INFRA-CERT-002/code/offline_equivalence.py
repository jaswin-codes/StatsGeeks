...
def main():
    ...
    for name,path in paths.items():
        ...
        for index in SELECTED:
            ...
            v.require(generated_sha==v.sha(saved_path),'NPZ byte divergence: '+e['episode_id'])
    ...
    for old,new in zip(record['runs']['original']['episodes'],record['runs']['optimized_native']['episodes']):
        v.require({k:a for k,a in old.items() if k!='compute_seconds'}=={k:a for k,a in new.items() if k!='compute_seconds'},'Original/native verification divergence')
    record['status']='OFFLINE_PREDICTION_EQUIVALENCE_PASS';record['total_seconds']=time.perf_counter()-start
