...
import numpy as np
ROOT=pathlib.Path(__file__).resolve().parents[3];RUN=ROOT/'working/G4C-E2-007';MH='9d493d99f60098cb22fd781c51e2d2907d6831df72cb2524700293aa1a1bbf32'
def sha(p):
...
def main():
 ...
 result={'status':'PASS','run_id':'G4C-E2-007','independent_source_state':True,'exact_predictions':60,'independent_score_confusion_records':60,'historical_raw_exact':60,'historical_u0_exact':60,'same_query_alignment':True,'packet_hash_reconstruction':60,'rng_manifest_replay':60,'worker_teardown':60,'fresh_state_probes':3,'protected_33':'PASS','original_clean':True,'manifest_sha256':MH,'verdict':verdict,'seconds':time.perf_counter()-started,'verifier_sha256':sha(__file__),'label_boundary':'Verifier privileged; candidate workers limited to source state/current support and never receive query labels/manifest'}
