...
for _k in ('OPENBLAS_NUM_THREADS','OMP_NUM_THREADS','MKL_NUM_THREADS','NUMEXPR_NUM_THREADS'):
    os.environ[_k]='1'
import datetime,hashlib,json,pathlib,pickle,platform,subprocess,sys,tempfile,time,traceback
import numpy as np
ROOT=pathlib.Path(__file__).resolve().parents[3];RUN=ROOT/'working/G4C-E2-003'
...
WORKERS={'source':'g4c_source_worker.py','episode':'g4c_episode_worker.py','preflight':'gate4b_preflight_worker.py'}
STATIC=[]
...
def preexisting_integrity():
 ...
 for path,anchor in [(ROOT/'working/G4C-E2-001/artifact_hash_manifest.json','46cdbabcd98754f9b879250090359d29a8a64861b32887ae8e7b24b423007436'),(ROOT/'working/G4C-E2-002/checkpoint/artifact_hash_manifest.json','72d5a082443b5f5e6e4eb7879d9e17c860163079b62f0af2ebd2eee959b9e302')]:
...
def call(packet,role,label):
 ...
 cmd=['wsl','-d','Ubuntu','--exec','/usr/bin/python3','-B',linux(ROOT/'working/g4c_linux_call.py'),linux(temp),linux(ROOT/'working'/WORKERS[role]),role,linux(out)]
...
def main():
 ...
 authorization={'run_id':'G4C-E2-003','authorization':'Explicit Team Lead E2-only execution authorization in current user message','timestamp_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'candidate':'E2 only','E3_E5':False,'parameters':PARAMS,'manifest_hash':MH,'rf_hash':RFH,'preparation_preflight':sha(RUN/'checkpoint/preflight.json'),'metadata_events_at_start':metadata_start,'notebook4_rerun':False}
